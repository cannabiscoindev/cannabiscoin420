                              ...
                              std::vector<uint256>& vHashTxnToUncache)
{
...

    {
        ...
        {
            LogPrint("mempool", "replacing tx %s with %s for %s CANN additional fees, %d delta bytes\n",
                  ...
                  CCoinsViewCache& view, const CChainParams& chainparams, bool fJustCheck)
{
    ...
    // Cannabiscoin 0.8 did not use IsSuperMajority to enforce v2 BIP34 which made BIP30 obsolete.
    // The 0.13 release will enforce all IsSuperMajority soft forks, BIP34 height and hash can be set
    // in the next release once enabled across the network. After the BIP34 height BIP30 can be skipped.
...
bool ContextualCheckBlock(const CBlock& block, CValidationState& state, CBlockIndex * const pindexPrev)
{
    ...
    if (block.nVersion >= 3 && IsSuperMajority(3, pindexPrev, consensusParams.nMajorityEnforceBlockUpgrade, consensusParams))
    {
